function [A, B] = covSum(covfunc, logtheta, x, z, u)
% [A, B] = covSum(covfunc, logtheta, x, z, u);
%
...
switch nargin
    ...
    case 5                      % compute derivative matrix for testxtraining set
        i = v(u);                                       % which covariance function
        j = sum(v(1:u)==i);                    % which parameter in that covariance
        f = covfunc(i);
        if iscell(f{:}), f = f{:}; end        % dereference cell array if necessary
        A = feval(f{:}, logtheta(v==i), x, z, j);                 % compute derivative
end
