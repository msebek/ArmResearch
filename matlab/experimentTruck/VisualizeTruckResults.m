...
zllTrainingTraces = cell( numMethods, numFolds );
nllTrainingTraces = cell( numMethods, numFolds );
fllTrainingTraces = cell( numMethods, numFolds );

fmeTrain = zeros( numMethods, numFolds );
fllTrain = zeros( numMethods, numFolds );
zllTrain = zeros( numMethods, numFolds );
% smeTrain = zeros( numMethods, numFolds );
% sllTrain = zeros( numMethods, numFolds );
nllTrain = zeros( numMethods, numFolds );

fmeTest = zeros( numMethods, numFolds );
fllTest = zeros( numMethods, numFolds );
zllTest = zeros( numMethods, numFolds );
% smeTest = zeros( numMethods, numFolds );
% sllTest = zeros( numMethods, numFolds );
nllTest = zeros( numMethods, numFolds );
...
for n = 1:numFolds
    for j = 1:numMethods-1
        ...
        fmeTest(j,n) = eval( [methodTest, '.filter.x_avg_err'] );
        fllTest(j,n) = eval( [methodTest, '.filter.x_avg_ll'] );
        zllTest(j,n) = eval( [methodTest, '.filter.z_avg_ll'] );
        %             smeTest(j,k) = eval( [methodTest, '.smoother.x_avg_err'] );
        %             sllTest(j,k) = eval( [methodTest, '.smoother.x_avg_ll'] );
        nllTest(j,n) = eval( [methodTest, '.true.n_avg_ll'] );

        zTemp = eval( ['[res.', methods{j}, 'Results(', num2str(n), ').train.evals.filter]'] );
        nTemp = eval( ['[res.', methods{j}, 'Results(', num2str(n), ').train.evals.true]'] );
        zllTrainingTraces{j,n} = [zTemp.z_avg_ll];
        nllTrainingTraces{j,n} = [nTemp.n_avg_ll];
        fllTrainingTraces{j,n} = [zTemp.x_avg_ll];
        fmeTrainingTraces{j,n} = [zTemp.x_avg_err];

        fmeTrain(j,n) = eval( [methodTrain, '.filter.x_avg_err'] );
        fllTrain(j,n) = eval( [methodTrain, '.filter.x_avg_ll'] );
        zllTrain(j,n) = eval( [methodTrain, '.filter.z_avg_ll'] );
        %             smeTrain(j,k) = eval( [methodTrain, '.smoother.x_avg_err'] );
        %             sllTrain(j,k) = eval( [methodTrain, '.smoother.x_avg_ll'] );
        nllTrain(j,n) = eval( [methodTrain, '.true.n_avg_ll'] );


    end
    fmeTest(end,n) = res.baselineResults(n).test_eval.adaptive.x_avg_err;
    fllTest(end,n) = res.baselineResults(n).test_eval.adaptive.x_avg_ll;
    zllTest(end,n) = res.baselineResults(n).test_eval.adaptive.z_avg_ll;
    nllTest(end,n) = res.baselineResults(n).test_eval.adaptive.n_avg_ll;

    fmeTrain(end,n) = res.baselineResults(n).train_eval.adaptive.x_avg_err;
    fllTrain(end,n) = res.baselineResults(n).train_eval.adaptive.x_avg_ll;
    zllTrain(end,n) = res.baselineResults(n).train_eval.adaptive.z_avg_ll;
    nllTrain(end,n) = res.baselineResults(n).train_eval.adaptive.n_avg_ll;
end

fmeTestFolded = mean( fmeTest, 2 );
fllTestFolded = mean( fllTest, 2 );
zllTestFolded = mean( zllTest, 2 );
nllTestFolded = mean( nllTest, 2 );

fmeTrainFolded = mean( fmeTrain, 2 );
fllTrainFolded = mean( fllTrain, 2 );
zllTrainFolded = mean( zllTrain, 2 );
nllTrainFolded = mean( nllTrain, 2 );
